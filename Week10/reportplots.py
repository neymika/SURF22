...
def main():
    ...
    histsigmflosses = pd.read_csv("../Week9/svrgcoslosses.csv", header=0)
    histsigmflosses = histsigmflosses.apply(lambda x: pd.Series(x.dropna().values))
    histsigmffuncs = pd.read_csv("../Week9/svrgcosfunc.csv", header=0)
    histsigmffuncs = histsigmffuncs.apply(lambda x: pd.Series(x.dropna().values))
    histsigmfolosses = pd.read_csv("../Week9/svrgcosolosses.csv", header=0)
    histsigmfolosses = histsigmfolosses.apply(lambda x: pd.Series(x.dropna().values))
    histsigeuclosses = pd.read_csv("../Week9/svrgeuclosses.csv", header=0)
    histsigeuclosses = histsigeuclosses.apply(lambda x: pd.Series(x.dropna().values))
    histsigeucfuncs = pd.read_csv("../Week9/svrgeucfunc.csv", header=0)
    histsigeucfuncs = histsigeucfuncs.apply(lambda x: pd.Series(x.dropna().values))
    histsigeucolosses = pd.read_csv("../Week9/svrgeucolosses.csv", header=0)
    histsigeucolosses = histsigeucolosses.apply(lambda x: pd.Series(x.dropna().values))
    ...
    with mpl.rc_context(params):
        ...
        for i in range(3):
            dictkey = str((i+1)*10)
            lastindex = histsigmffuncs[dictkey].notna()[::-1].idxmax()
            xrange = range(0, lastindex+1)

            axs[i].plot(xrange, histsigmfolosses[dictkey][0:lastindex+1], '-', color=colors[0])
        ...
        plt.savefig("svrgmf_best_oloss.png", bbox_inches='tight')
        ...
        for i in range(3):
            ...
            olastindex = histsigmffuncs[odictkey].notna()[::-1].idxmax()
            xrange = range(0, olastindex+1)

            axs[i].plot(range(0, lastindex+1), \
            histsigolosses[dictkey][0:lastindex+1], '-', alpha=.65, \
            label=rf'Random Fixed SVRG', color=colors[0])
            axs[i].plot(xrange, histsigmfolosses[odictkey][0:olastindex+1], '-.', \
            alpha=.8, label=rf'Multifidelity Fixed SVRG', color=colors[1])
            axs[i].plot(range(0, lastindex+1), 1e-4*np.ones(lastindex+1), \
            '--', markeredgecolor="none", alpha=.6, \
            label=r'Convergence $\tau = 10^{-4}$', color=colors[3])
            axs[i].set_yscale('log')
            axs[i].set_title(\
            rf'Fixed $\eta = {stepsizes[i]}$', \
            pad=15,fontsize=10,color=lbl)
        ...
        for i in range(3):
            dictkey = str((i+1)*10)
            odictkey = str((i+1)*10)
            lastindex = histsigeucfuncs[dictkey].notna()[::-1].idxmax()
            olastindex = histsigmffuncs[odictkey].notna()[::-1].idxmax()
            xrange = range(0, olastindex+1)
            print(histsigeucolosses[dictkey])

            axs[i].plot(range(0, lastindex+1), \
            histsigeucolosses[dictkey][0:lastindex+1], '-', alpha=.65, \
            label=rf'Multifidelity Euclidean-Based $\rho$ SVRG', color=colors[0])
            axs[i].plot(xrange, histsigmfolosses[odictkey][0:olastindex+1], '-.', \
            alpha=.8, label=rf'Multifidelity Cosine-Based $\rho$ SVRG', color=colors[1])
            axs[i].plot(range(0, lastindex+1), 1e-4*np.ones(lastindex+1), \
            '--', markeredgecolor="none", alpha=.6, \
            label=r'Convergence $\tau = 10^{-4}$', color=colors[3])
            axs[i].set_yscale('log')
        ...
        plt.setp(axs[:], ylabel=r'||$\nabla$L(w)|| Values')
        plt.legend(loc = "upper center",bbox_to_anchor=(-0.6,1.05), ncol=3, title="Optimization Methods")
        plt.savefig("svrgmf_best_comp.png", bbox_inches='tight')
