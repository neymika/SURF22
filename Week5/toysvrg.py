...
def multipsi(sig, inds, lam=1e-4):
    sumi = lam*(sig.T @ sig)
    chosenxi = np.take(xi, inds, axis=0)
    chosenyi = np.take(yi, inds, axis=0)
...
def multiphipr(df, xk, pk, a, inds):
    return (df(xk + a*pk, inds).T @ pk)

def backtracing(f, df, xk, pk, mu1, alf0, rho, multilin=False, inds=None):
    alf = alf0
    ahist = alf0
    costs = 0

    if inds is not None:
        if type(inds) is np.int64:
            mult = 1
        else:
            mult = inds.shape[0]
    ...
    if multilin:
        for i in range(100):
            costs += 3*mult
            if multiphi(f, xk, pk, alf, inds) <= multiphi(f, xk, pk, 0, inds) + \
            mu1*alf*multiphipr(df, xk, pk, 0, inds):
                break
            alf = rho*alf
...
def sgdescent(f, df, x0, etait, epochs=1000, miter=50, tau=1e-4, linesearch=True):
    ...
    while losses[-1] > tau:
        ...
        costs = np.append(costs, costs[k])
        for m in range(maxes):
            if linesearch:
                # if miter > 1:
                #     indchosen = chosen[m:m+miter-1]
                #     pk = -dfmultipsi(xk, indchosen)
                #     alfk, cost = backtracing(multipsi, dfmultipsi, xk, pk, mu1, \
                #     alfk, rho, multilin=True, inds=indchosen)
                # else:
                pk = -df(xk, chosen[m])
                alfk, cost = backtracing(f, df, xk, pk, mu1, alfk, \
                rho, multilin=True, inds=chosen[m])
                costs[k+1] += cost
            else:
                if k >= 100:
                    alfk = etait(k)
    ...
    return xhist, losses, ahist, olosses, costs
...
def svrgdescent(f, df, x0, etait, tau=1e-4, epochs=1000, miter=100, naive=True, func=False, linesearch=True):
    k = 0
    xs = np.copy(x0)
    xhist = np.array([xs])
    reset = False
    ...
    for s in range(epochs):
        ...
        for m in range(maxes):
            if m % miter == 0:
                # DOES NOT WORK! OVERFLOW/SMALL ETA RESULTS
                # if linesearch:
                    # if miter > 1:
                    #     indchosen = chosen[m:m+miter-1]
                    #     pk = -(dfmultipsi(xk, indchosen) - \
                    #     dfmultipsi(xs, indchosen) + mu)
                    #     eta = backtracing(multipsi, dfmultipsi, xk, pk, mu1, \
                    #     eta, rho, multilin=True, inds=indchosen)
                    # else:
                    #     pk = -(df(xk, chosen[m]) - df(xs, chosen[m]) + mu)
                    #     alfk = backtracing(f, df, xk, pk, mu1, eta, \
                    #     rho, multilin=True, inds=chosen[m])
...
def steepestdescent(f, df, x0, tau, alf0, mu1, rho, epochs =1100):
    ...
    while np.linalg.norm(dfloss(xk), ord=2)/np.linalg.norm(xk, ord=2) > tau:
        ...
        if k != 0:
            ...
            pk = -dk
        costs = np.append(costs, costs[k])
        alfk, cost = backtracing(f, df, xk, pk, mu1, alfk, rho)
        costs[k+1] += cost
        # if k > 100:
        #     alfk = 1/k
        # else:
...
def main():
    ...
    for i in range(len(minibatches)):
        ...
        test_start_iter = timeit.default_timer()
        sigmafound, siglosses, sigfuncs, sigolosses, sigcosts = \
        sgdescent(psi, dfpsi, initialguess, firsteta, epochs=200, \
        miter=minibatches[i], tau=1e-4)
        test_end_iter = timeit.default_timer()
        print(test_end_iter - test_start_iter )
        print(sigolosses[-1])
        print()
        ...
        histsigsgdcosts[minibatches[i]] = sigcosts

    print("Performing Steepest Descent")
    test_start_iter = timeit.default_timer()
    gdsigmafound, gdsiglosses, gdsigfuncs, gdsigolosses, gdsigcosts = \
    steepestdescent(loss, dfloss, initialguess, 1e-4, 1, 1e-5, .5, epochs=200)
    test_end_iter = timeit.default_timer()
    print(test_end_iter - test_start_iter )
    print()
    ...
    with mpl.rc_context(params):
        ...
        axs[0].plot(gdsigcosts/1000, gdsigfuncs, '-.', markeredgecolor="none", \
        label=f'Gradient Descent')
        axs[0].set_yscale('log')

        axs[1].plot(gdsigcosts/1000, gdsigolosses, '-.', markeredgecolor="none", \
        label=f'Gradient Descent')
        axs[1].set_yscale('log')

        axs[2].plot(gdsigcosts/1000, gdsiglosses, '-.', markeredgecolor="none", \
        label=f'Gradient Descent')
        ...
        for mini in range(len(minibatches)):
            ...
            # print(histsigsgdcosts[minibatches[mini]].shape)
            # print(sizes)
            axs[0].plot(histsigsgdcosts[minibatches[mini]]/1000, \
            histsigsgdfuncs[minibatches[mini]], '-.', markeredgecolor="none",  \
            label=f'minibatch size = {minibatches[mini]}')

            axs[1].plot(histsigsgdcosts[minibatches[mini]]/1000, \
            histsigsgdolosses[minibatches[mini]], '-.', markeredgecolor="none", \
            label=f'minibatch size = {minibatches[mini]}')

            axs[2].plot(histsigsgdcosts[minibatches[mini]]/1000, \
        ...
        plt.savefig("toy_sgdcosts.png", bbox_inches='tight')
