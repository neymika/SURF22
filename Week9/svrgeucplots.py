...
def main():
    ...
    histsiglosses = pd.read_csv("svrgeuclosses.csv", header=0)
    histsiglosses = histsiglosses.apply(lambda x: pd.Series(x.dropna().values))
    histsigfuncs = pd.read_csv("svrgeucfunc.csv", header=0)
    histsigfuncs = histsigfuncs.apply(lambda x: pd.Series(x.dropna().values))
    histsigolosses = pd.read_csv("svrgeucolosses.csv", header=0)
    ...
    with mpl.rc_context(params):
        ...
        plt.savefig("toy_svrgeuc_oloss.png", bbox_inches='tight')
        ...
        plt.savefig("toy_svrgeuc_func.png", bbox_inches='tight')
