...
def main():
    ...
    for i in range(len(minibatches)):
        ...
        dfpsi, initialguess, firsteta, tau=1e-4, epochs=int(200*2000/minibatches[i]), miter=minibatches[i], \
    ...
    for i in range(len(minibatches)):
        ...
        firsteta, epochs=200*2000/minibatches[i], miter=minibatches[i], tau=1e-4)
        test_end_iter = timeit.default_timer()
        print(test_end_iter - test_start_iter )
        print()
    ...
    test_start_iter = timeit.default_timer()
    gdsigmafound, gdsiglosses, gdsigfuncs, gdsigolosses = steepestdescent(loss, dfloss, initialguess, 1e-4, 1, 1e-5, .5, epochs=2000)
    test_end_iter = timeit.default_timer()
    print(test_end_iter - test_start_iter )
    print()
    ...
    try:
        ...
        export_png(style(p), filename="unknownn_func.png")
        ...
        p.legend.label_text_font_size = '10px'
        p.legend.background_fill_alpha = 0.2
        export_png(style(p), filename="unknownn_oloss.png")
        ...
        p.legend.label_text_font_size = '10px'
        p.legend.background_fill_alpha = 0.2
        export_png(style(p), filename="unknownn_loss.png")
        ...
        for mini in minibatches:
            ...
            label=f'SVRG with minibatch size = {mini}')
        ...
        plt.savefig("unknownn_func.png", bbox_inches='tight')
        ...
        plt.legend(bbox_to_anchor=(1,1.25), loc='upper left', ncol=1, title="Optimization Methods")
        plt.savefig("unknownn_loss.png", bbox_inches='tight')
        ...
        plt.ylabel(r"||$\nabla$L(w)|| Values")
        plt.legend(bbox_to_anchor=(1,1.25), loc='upper left', ncol=1, title="Optimization Methods")
        plt.savefig("unknownn_oloss.png", bbox_inches='tight')
