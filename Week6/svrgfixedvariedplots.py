...
def main():
    ...
    histsiglosses = pd.read_csv("svrgfixedlosses.csv", header=0)
    histsiglosses = histsiglosses.apply(lambda x: pd.Series(x.dropna().values))
    histsigfuncs = pd.read_csv("svrgfixedfunc.csv", header=0)
    histsigfuncs = histsigfuncs.apply(lambda x: pd.Series(x.dropna().values))
    histsigolosses = pd.read_csv("svrgfixedolosses.csv", header=0)
    ...
    with mpl.rc_context(params):
        ...
        for i in range(len(stepsizes)):
            ...
            axs[i].plot(range(0, histsigfuncs[dictkey].shape[0]), histsigolosses[dictkey], '-')
        ...
        plt.savefig("toy_svrgfixed_oloss.png", bbox_inches='tight')
        ...
        plt.savefig("toy_svrgfixed_func.png", bbox_inches='tight')
