...
def main():
    ...
    histsiglosses = pd.read_csv("svrgfasterlosses.csv", header=0)
    histsiglosses = histsiglosses.apply(lambda x: pd.Series(x.dropna().values))
    histsigfuncs = pd.read_csv("svrgfasterfunc.csv", header=0)
    histsigfuncs = histsigfuncs.apply(lambda x: pd.Series(x.dropna().values))
    histsigolosses = pd.read_csv("svrgfasterolosses.csv", header=0)
    ...
    with mpl.rc_context(params):
        ...
        plt.savefig("toy_svrg_faster_oloss.png", bbox_inches='tight')
        ...
        plt.savefig("toy_svrg_faster_func.png", bbox_inches='tight')
